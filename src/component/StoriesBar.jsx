...
function StoriesBar() {
  ...
  return (
    <div className='w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-sclollbar overflow-x-auto px-4'>
        ...
        {/* Story Cards  */}
         {
            storis.map((story , index) => (
                <div key={index} className={` relative rounded-lg shadow min-w-30 max-w-30 max-h-40 cursor-pointer hover:shadow-lg transition-all duration-200 bg-gradient-to-b from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 active:scale-95`}>
                    ...
                    <p className=' absolute top-18 left-3 text-white/60 text-sm truncate mex-w-24'>{story.content}</p>
                    <p className=' absolute text-white bottom-1 right-2 z-10 text-xs'>{moment(story.createdAt).fromNow()}</p>
                    {
                        story.media_type !== 'text' && (
                            ...
                            <div className='absolute inset-0 z-1 rounded-lg bg-black overflow-hidden'>
                                    {
                                        storis.media_type === "image" ? 
                                        <img src={story.media_url} alt="" className='h-full w-full object-cover hover:scale-110 opacity-70 transition duration-500 hover:opacity-80'/>
                                        :
